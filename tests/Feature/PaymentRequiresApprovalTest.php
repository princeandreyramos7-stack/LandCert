...
class PaymentRequiresApprovalTest extends TestCase
{
    ...
    public function test_record_payment_page_is_blocked_while_awaiting_approval(): void
    {
        ...
        // The id-bearing route remembers the record and bounces to the clean
        // URL; the refusal happens there.
        $this->actingAs($user)
            ->get("/receipt/upload/{$request->id}")
            ->assertRedirect('/upload-receipt');
    ...
    public function test_order_of_payment_is_blocked_while_awaiting_approval(): void
    {
        ...
        $this->actingAs($user)
            ...
            ->assertRedirect('/order-of-payment');
    ...
    /**
     * Regression: both refusals on the order of payment used back(). The page
     * is reached by a bounce from the id-bearing route, so "back" was that
     * route - which bounced to the page again, for ever.
     */
    public function test_the_order_of_payment_refusals_never_send_the_browser_back_into_the_bounce(): void
    {
        $user = $this->userOf('applicant');
        $request = $this->applicationWithStatus($user, 'reviewed');

        // Arriving from the bounce itself, with no better referer to fall back on.
        $this->actingAs($user)
            ->from("/my-applications/{$request->id}/order-of-payment")
            ->get('/order-of-payment')
            ->assertRedirect(route('my-applications'));

        // Someone else's application is refused outright.
        $this->actingAs($this->userOf('applicant'))
            ->withSession(['clean_page.order-of-payment' => $request->id])
            ->from("/my-applications/{$request->id}/order-of-payment")
            ->get('/order-of-payment')
            ->assertForbidden();
    }
