...
export function SuperAdminDashboard({
    ...
    const statCards = [
        {
            label: "Total Users",
            sub: `${systemStats.total_applicants || 0} applicants · ${systemStats.total_admins || 0} admins`,
            value: systemStats.total_users || 0,
            icon: Users,
            border: "border-l-[#0d1f5c]",
            iconBg: "bg-[#0d1f5c]/10",
        ...
        },
        {
            ...
            sub: "Awaiting action",
            value: systemStats.pending_requests || 0,
            icon: Clock,
            ...
            {showAnalytics && (
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    {statCards.map((s, i) => (
                        <Card
                            key={i}
                            className={`border-l-4 ${s.border} bg-white shadow-sm hover:shadow-md transition-shadow`}
                        >
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                <CardTitle className="text-xs font-bold text-gray-500 uppercase tracking-wide">
                                    {s.label}
                                </CardTitle>
                                <div
                            ...
                            <CardContent>
                                <div className="text-2xl sm:text-3xl font-black text-[#0d1f5c]">
                                    {s.value}
                                </div>
                            ...
                            <div className="flex flex-col md:flex-row items-center gap-8">
                                ...
                                <div className="flex-1 text-center md:text-left space-y-3">
                                    <div className="flex items-center gap-2 justify-center md:justify-start">
                                        <div className="w-1 h-4 rounded-full bg-[#d4a017]" />
                                        <p className="text-[#d4a017] text-xs font-black tracking-widest uppercase">
                                            Zoning Administrator — CPDO Ilagan
                                            City
                                        </p>
                                    </div>
                                    ...
                                    <p className="text-gray-500 text-sm max-w-xl mx-auto md:mx-0">
                                        ...
                                        , geographic breakdowns and the admin
                                        workflow log.
                                    </p>
                                    ...
                                    <div className="flex flex-wrap gap-2 pt-1 justify-center md:justify-start">
                                        {[
                                            {
                                                icon: Clock,
                                                text: `${systemStats.pending_requests || 0} pending`,
                                                cls: "text-yellow-700 bg-yellow-50 border-yellow-200",
                                            },
                                            {
                                                icon: FileText,
                                                text: `${systemStats.total_requests || 0} total applications`,
                                                cls: "text-blue-700 bg-blue-50 border-blue-200",
                                            },
                                            {
                                                icon: Users,
                                                text: `${systemStats.total_users || 0} users`,
                                                cls: "text-[#0d1f5c] bg-[#0d1f5c]/5 border-[#0d1f5c]/15",
                                            },
                                        ].map((f, i) => (
                                            <span
                                                key={i}
