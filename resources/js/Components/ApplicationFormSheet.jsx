...
/**
 * The Application Form itself — both sheets, without the page around them.
 *
 * Lifted out of the Print Form page so the applicant report can show the same
 * form. Nothing is kept on file for it (the notarized copy the applicant hands
 * in is a requirement scan, and a separate thing): it is drawn from the
 * application each time, so it is drawn here, once, for both.
 */

/* Fixed control number for all applications */
export const FORM_CONTROL_NO = 'CPD-001-0';
...
/* ════════════════════════════════════════════════════
   SHEET CSS — everything under .pf-page, so the form
   can sit inside another page without touching it.
   ════════════════════════════════════════════════════ */
const SHEET_CSS = `
/* ── screen wrapper ── */
.pf-page {
...
/**
 * @param application  the form data the Print Form page receives
 * @param compact      drop the A4 height and chrome (for previews)
 * @param wrap         (page, key) => node — wraps each of the two sheets, for a
 *                     host that wants them as separate pages of its own
 */
export default function ApplicationFormSheet({ application: a, compact = false, wrap = (page) => page }) {
    ...
    const pageClass = compact ? "pf-page sheet--compact" : "pf-page";
    ...
    const checklistPage = (
            <div className={pageClass} style={{ pageBreakBefore: "always", padding: "40pt 60pt" }}>
