...
import { FileText, FileCheck, Stamp, Wallet, CheckCircle2, XCircle } from "lucide-react";
...
const ALL_TILES = [
    ...
    { key: "approved",             label: "For Payment",         sub: "approved, fee unpaid",    icon: Wallet,    tone: "text-violet-800", ring: "ring-violet-500", wash: "bg-violet-50",    chip: "bg-violet-500 text-white", roles: ["admin"] },
...
export function ApplicationsStats({ requests, active = "all", onSelect, role = "admin" }) {
    ...
    return (
        <div className={`grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3 ${TILES.length <= 4 ? "xl:grid-cols-4" : "xl:grid-cols-6"}`}>
            {TILES.map((t) => {
