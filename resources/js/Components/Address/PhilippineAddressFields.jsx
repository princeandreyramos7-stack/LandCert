...
/**
 * Region → Province → City/Municipality → Barangay → Street.
 *
 ...
 * … `${prefix}_street`, so they post with the rest of the form and the server
 * composes the address line from them (App\Support\PhilippineAddress).
 */
...
export function PhilippineAddressFields({
    ...
    const latest = useRef({ provinces: null, cities: null, barangays: null });

    const load = useCallback(async (level, url, key) => {
        latest.current[level] = key;
        setLoading((l) => ({ ...l, [level]: true }));
        try {
            const data = await fetchList(url);
            if (latest.current[level] !== key) return;
            ...
            if (latest.current[level] === key) setLoading((l) => ({ ...l, [level]: false }));
        }
    }, []);

    useEffect(() => { load("regions", route("psgc.regions"), "all"); }, [load]);

    // Each level follows the one above it — on first render too, so an
    // address already filled in comes back with all four lists populated.
    ...
    useEffect(() => {
        ...
        load("provinces", route("psgc.provinces", region), region);
    }, [region, load]);
        ...
        <fieldset className="rounded-xl border border-gray-200 bg-gray-50/50 p-4">
            ...
            {failed && (
                <p className="mb-3 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-xs text-rose-700">
                    The address list could not be loaded. Check your connection and pick the {failed === "regions" ? "region" : failed.replace(/s$/, "")} again.
                </p>
            )}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
                <Row
                    part="region_code"
                    label="Region"
                    options={lists.regions.map((r) => ({ value: r.code, label: r.name, hint: r.short_name }))}
                ...
                <Row
                    ...
                    options={lists.provinces.map((p) => ({
                        value: p.code,
                        label: p.name,
                        hint: PROVINCE_LABEL[p.kind],
                    }))}
                    loadingLevel="provinces"
                    placeholder="Select province"
                    waitingFor={region ? null : "region"}
                    emptyText="No provinces in this region"
                />
                <Row
                    part="city_code"
