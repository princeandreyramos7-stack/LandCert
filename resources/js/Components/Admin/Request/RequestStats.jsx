...
const CARDS = [
    { key: "all",      label: "Total Applications",      sub: "All submissions",         bg: "bg-[#0d1f5c]/5", text: "text-[#0d1f5c]", icon: FileText,     iconBg: "bg-[#0d1f5c]" },
...
export function RequestStats({ stats, onFilterChange }) {
    ...
    return (
        ...
        // items-stretch + h-full keeps the pair in a row the same height when
        // one label wraps to two lines and its neighbour does not.
        <div className="grid grid-cols-2 items-stretch gap-3 sm:gap-4 lg:grid-cols-4">
            {CARDS.map(c => (
                <Card
                    ...
                    className={`h-full cursor-pointer ${c.bg} border-0 transition-shadow hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0d1f5c] focus-visible:ring-offset-2`}
                ...
                >
                    <CardContent className="p-3 sm:p-4">
                        {/* The icon sits under the label on a phone instead of
                           beside it: two across leaves the text roughly 110px,
                           and a badge stealing 30px of that broke
                           "Application Approved" onto three lines. */}
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                            <div className="min-w-0 order-2 sm:order-1">
                                <p className={`mb-1 break-words text-[10px] font-bold uppercase leading-tight tracking-wide sm:text-xs ${c.text} opacity-70`}>
                                    {c.label}
                                </p>
                            ...
                            <div className={`order-1 w-fit shrink-0 rounded-lg p-1.5 sm:order-2 sm:p-2 ${c.iconBg}`}>
