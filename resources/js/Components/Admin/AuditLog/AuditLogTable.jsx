...
/**
 * The log as a table of events: when (as "3 hours ago", with the exact time
 * beneath), who (with their role), what they did (coloured by the kind of
 * thing it was), what it was done to, and where from. A row opens its full
 * record when clicked.
 */
    ...
    if (rows.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-gray-200 px-4 py-14 text-center">
                ...
                <p className="text-sm font-semibold text-gray-700">No activity matches</p>
                <p className="text-xs text-gray-400">Try a wider date range or clear the filters.</p>
            </div>
        );
    }
            ...
            <table className="w-full min-w-[56rem] text-sm">
                <thead className="sticky top-0 z-10 bg-white">
                    <tr className="border-b border-gray-200 text-left text-[11px] font-bold uppercase tracking-wide text-[#0d1f5c]">
                        ...
                        <th className="px-3 py-2.5 text-right"><span className="sr-only">Open</span></th>
                    </tr>
                </thead>
                <tbody>
                    {rows.map((log, i) => {
                        ...
                        return (
                            ...
                            >
                                <td className="whitespace-nowrap px-4 py-3 sm:px-3">
                                    <div className="text-xs font-semibold text-gray-900">{timeAgo(log.created_at)}</div>
                                    <div className="mt-0.5 text-[11px] text-gray-400">{formatDate(log.created_at)}</div>
                                </td>
                                <td className="px-3 py-3">
                                    <div className="flex items-center gap-2.5">
                                        <div
                                            ...
                                            aria-hidden="true"
                                        >
                                            {initialsOf(log.user_name || "System")}
                                        </div>
                                        <div className="min-w-0">
                                            <div className="truncate text-xs font-semibold text-gray-900">{log.user_name || "System"}</div>
                                            <div className="truncate text-[11px] text-gray-400">{roleLabel(role)}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td className="px-3 py-3">
                                    <p className="line-clamp-2 max-w-md text-xs text-gray-700" title={log.description}>
                                        {log.description}
                                    </p>
                                </td>
                                <td className="whitespace-nowrap px-3 py-3">
