...
class SuperAdminController extends Controller
{
    ...
    public function archiveRequest($id)
    {
        ...
        // Moves the request (and its payments) between the cached "live" and
        // "archived" buckets - see DashboardCacheService::rememberPayments.
    ...
    public function viewApplication($id): Response
    {
        ...
        // Get full requirements list for this project type
        ...
        // Build uploaded requirements with ALL documents (not filtered)
    ...
    public function auditLogs(Request $request): Response
    {
        ...
        if ($request->filled('model_type')) {
            $query->where('audit_logs.model_type', $request->model_type);
        }

        if ($request->filled('date_from')) {
            $query->where('audit_logs.created_at', '>=', $request->date_from);
        }

        if ($request->filled('date_to')) {
            $query->where('audit_logs.created_at', '<=', $request->date_to . ' 23:59:59');
        }
    ...
    public function updatePayment(Request $request, $paymentId)
    {
        ...
        // The Payments page caches these figures for a short while (see
        // DashboardCacheService::rememberPayments) - an edit made here must
        // show up right away, not up to 20s later.
        $this->cacheService->clearCache();
